...
export class CandidateEditComponent implements OnInit, OnDestroy {
      ...
      if (id) {
        this.candidateService.get(id).subscribe((candidate: any) => {
          if (candidate) {
            ...
            this.attachmentsDS.data = candidate._embedded ? candidate._embedded.attachments : null;
          } else {
  ...
  isMobile(): boolean {
    ...
    return this.checkIsMobile(userAgent) || this.checkIsMobile(vendor)
  }

  checkIsMobile(a: string): boolean {
